.PHONY: all help build clean validate lint
...
all: clean build ## Clean, then build the thesis PDF (default)
...
build: ## Build the thesis PDF into dist/
	mkdir -p dist
	mkdir -p build
...
validate: ## Pre-flight: draft compile + resolved log scan for silent errors
	mkdir -p build
...
lint: ## Run chktex stylistic linting over src/*.tex
	find src -name '*.tex' -print0 | xargs -0 chktex -l .chktexrc

clean: ## Remove build artifacts and editor backup files
	rm -f dist/* build/*
	find . -iname "*~" -exec rm '{}' ';'
